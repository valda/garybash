...
class BackupSettings(BaseBackupSettings):
    def __init__(self, parent=None, path=None, quit=False, backup_images=None):
        ...
        for path, tmpdir in (
              (dirs['mopy'].join('Data'),                 'Fallout New Vegas\\Mopy\\Data'),
              (dirs['mopy'].join('Data','Actor Levels'),  'Fallout New Vegas\\Mopy\\Data\\Actor Levels'),
        ...
        if backup_images == 1: #changed images only
            tmpdir = GPath('Fallout New Vegas\\Mopy\\Images')
...
class RestoreSettings(BaseBackupSettings):
    ...
    def Apply(self):
        ...
        if self.restore_images:
            restore_paths += (
                (dirs['mopy'].join('Images'),               'Fallout New Vegas\\Mopy\\Images'),)
