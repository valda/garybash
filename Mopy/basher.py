    ...
    def OnChar(self,event):
        ...
        if (event.GetKeyCode() == 127):
            ...
            (self.list.GetSelectedItemCount() == 1)
                ...
                if thisFile.cext != swapFile.cext: return
        ...
        if hitFlag == 32:
            ...
            if self.data.isSelected(fileName):
                ...
                if len(changed) > (fileName in changed):
                    ...
                    balt.showList(self,_('${count} Children deactivated:'),changed,10,_("Children Deactivated"))
            #--Select?
            else:
                try:
                    ...
                    if len(changed) > ((fileName in changed) + (GPath('Fallout3.esm') in changed)):
                        changed.remove(fileName)
                        changed = [x.s for x in changed]
                        balt.showList(self,_('${count} Masters activated:'),changed,10,_("Masters activated"))
...
class ModDetails(wx.Window):
    ...
    def __init__(self,parent):
        ...
        self.cancel.Disable()
        #--Bash tags
        self.allTags = sorted(('Body-F', 'Body-M', 'C.Climate', 'C.Light', 'C.Owner', 'C.Water', 'Delev', 'Eyes', 'Factions', 'Relations', 'Filter', 'Graphics', 'Hair', 'IIM', 'Invent', 'Names', 'NoMerge', 'NpcFaces', 'R.Relations', 'Relev', 'Sound', 'Stats', 'Voice-F', 'Voice-M','R.Teeth','R.Mouth','Roads'))
...
class PatchDialog(wx.Dialog):
    ...
    def Execute(self,event=None):
        ...
        try:
            ...
            if not bosh.modInfos.isSelected(patchName) and balt.askYes(self.parent,message,patchName.s):
                try:
                    oldFiles = bosh.modInfos.ordered[:]
...
class Mod_FlipSelf(Link):
    ...
    def AppendToMenu(self,menu,window,data):
        ...
        menu.AppendItem(menuItem)
        menuItem.Enable(bool(len(data) == 1 and data[0].cext[-1] == 'p'))
        #--Filetype
        fileInfo = self.fileInfo = window.data[data[0]]
        if fileInfo.isEsm():
...
class Mod_DecompileAll(Link):
    ...
    def AppendToMenu(self,menu,window,data):
        ...
        menu.AppendItem(menuItem)
        menuItem.Enable(len(self.data) == 1 and self.data[0] != 'Fallout3.esm')
...
class Mod_Formids_Replace(Link):
    ...
    def Execute(self,event):
        ...
        textDir = bosh.dirs['patches']
        #--File dialog
        textPath = balt.askOpen(self.window,_('Formid mapper file:'),textDir,
            '', '*Formids.csv')
        if not textPath: return
        (textDir,textName) = textPath.headTail
        #--Extension error check
        if textName.cext != '.csv':
            ...
            return
        #--Export
        progress = balt.Progress(_("Import Formids"))
        changed = None
        try:
...
class Mod_RemoveWorldOrphans(Link):
    ...
    def Execute(self,event):
        ...
        #--Log
        if orphans:
            balt.showOk(self.window,_("Orphan cell blocks removed: %d.") % (orphans,))
        else:
            balt.showOk(self.window,_("No changes required."))
...
class Mod_UndeleteRefs(Link):
    ...
    def AppendToMenu(self,menu,window,data):
        ...
        menu.AppendItem(menuItem)
        menuItem.Enable(len(self.data) == 1 and self.data != 'Fallout3.esm')
        ...
        try:
            ...
            for index,fileName in enumerate(map(GPath,self.data)):
                ...
                if undeleteRefs.fixedRefs:
                    ...
                    for formid in sorted(undeleteRefs.fixedRefs):
                        log('. %08X' % (formid,))
            progress.Destroy()
            if hasFixed:
    ...
    def __init__(self,parent,saveFile,recordTypes):
        ...
        for index,record in enumerate(saveFile.created):
            if record.recType == 'ENCH':
                self.enchantments[record.formid] = record.getTypeCopy()
...
def InitStatusBar():
    ...
    BashStatusBar.buttons.append( 
        App_Button(
            bosh.dirs['app'].join('Fallout3ModManager.exe'),
    ...
    BashStatusBar.buttons.append( 
        App_Tes4View(
            (bosh.dirs['app'].join('Tes4View.exe'),'-edit'),
            Image(r'images/brick_edit.png'),
            _("Launch Tes4Edit")))
    BashStatusBar.buttons.append( 
        App_Tes4View(
            (bosh.dirs['app'].join('Tes4View.exe'),'-translate'),
